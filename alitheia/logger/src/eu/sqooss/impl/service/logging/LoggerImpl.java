...
public class LoggerImpl implements Logger {
    ...
    public LoggerImpl(String name, LogWritersManager logStorage, LogConfiguration logConfig) {
        ...
        messageFormat = logConfig.getConfigurationProperty(name, 
                            LogConfigurationConstants.KEY_MESSAGE_FORMAT);
        fileName = logConfig.getConfigurationProperty(name, 
                       LogConfigurationConstants.KEY_FILE_NAME) + 
                   getFileExtension(messageFormat);
    ...
    /**
     * Certain keys update values in the log writer; handle them.
     */
    private void updateWriterProperties(String key, String value) {
        if (LogConfigurationConstants.KEY_ROTATION_FILE_LENGTH.equals(key)) {
            ...
            return; // TODO: is that acceptable style?
        } 
        if (LogConfigurationConstants.KEY_ROTATION_FILE_NUMBER.equals(key)) {
            try {
    ...
    private boolean configurationKeyValid(String key) {
        return LogConfigurationConstants.KEY_ROTATION_FILE_LENGTH.equals(key) ||
               LogConfigurationConstants.KEY_ROTATION_FILE_NUMBER.equals(key) ||
               LogConfigurationConstants.KEY_MESSAGE_FORMAT.equals(key) ||
    ...
    public void setConfigurationProperty(String key, String value, boolean isParentChange) {
        if (!configurationKeyValid(key)) {
            return;
        }

        if (!isParentChange) {
            logConfig.setConfigurationProperty(name, key, value);
            updateWriterProperties(key,value);
        }
    ...
    /**
     * Construct a string for the current logging style from the message body
     * and the given logging level.
     */
