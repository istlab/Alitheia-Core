...
public class MailingListThreadTest {

	static MailingListThread testObject;

	@BeforeClass
	public static void setUp() {
		testObject = new MailingListThread();
	}
	...
	@Test
	public void testIsFlameWarGetterSetter() {
		// set the value via the setter
		testObject.setIsFlameWar(true);

		// Pull out property in order to compare
		boolean actualValue = Whitebox.<Boolean> getInternalState(testObject,
				"isFlameWar");

		// Compare the expected with the actual result
		assertEquals(actualValue, true);

		boolean getValue = testObject.getIsFlameWar();
		// Compare the expected with the actual result
		assertEquals(getValue, true);
	}
