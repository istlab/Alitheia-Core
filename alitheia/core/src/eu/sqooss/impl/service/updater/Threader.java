/*
 * The following file is imported from the Mozilla Grendel project. 
 * As required by the MPL, the source file and the modifications 
 * made on it must not be re-licensed and the copyright cannot be
 * changed.  
 */
/*   
 ...
 * sets of messages are accessed via the IThreadable interface.
 */
class Threader {
     ...
     */
    public Threadable thread(Threadable threadable_root) {
        if (threadable_root == null)
            return null;
        id_table = new Hashtable();

        for (Enumeration e = threadable_root.allElements(); e.hasMoreElements();) {
            Threadable t = (Threadable) e.nextElement();
            if (!t.isDummy())
                buildContainer(t);
        }
        ...
        Threadable result = (root_node.child == null ? null
     ...
     *  - It walks the tree of threadables, and wraps each in a ThreadContainer
     * object.
     *  - It indexes each ThreadContainer object in the id_table, under the
     * message ID of the contained IThreadable.
     *  - For each of the IThreadable's references, it ensures that there is a
     * ThreadContainer in the table (an empty one, if necessary.)
     */
    private void buildContainer(Threadable threadable) {
        ...
        if (c != null) {
             ...
             * 
             * If there is already a threadable in it, then that means there are
             * two IThreadables with the same ID. Generate a new ID for this
             * one, sigh... This ID is only used to cause the two entries in the
             * hash table to not stomp each other.
             */
        ...
        ThreadContainer parent_ref = null;
        {
            ...
            for (int i = 0; i < L; i++) {
                ...
                ThreadContainer ref = (ThreadContainer) id_table
                        .get(ref_string);
        ...
        // At this point `parent_ref' is set to the container of the last
        // element
        ...
        if (c.parent != null) {
            /*
             * If it has a parent already, that's there because we saw this
             * message in a references field, and presumed a parent based on the
             * other entries in that field. Now that we have the actual message,
             * we can be more definitive, so throw away the old parent and use
             * this new one.
             * 
             ...
             * it has no references field, but some message referred to it as
             * the non-first element of its references. (Which would have been
             * some kind of lie...)
             */
    ...
    private void pruneEmptyContainers(ThreadContainer parent) {
        ...
        for (prev = null, container = parent.child, next = container.next; 
             container != null; 
             prev = container, container = next, next = (container == null ? null: container.next)) {

            if (container.threadable == null && container.child == null) {
                /*
                 * This is an empty container with no kids. Nuke it.
                 * 
                 * Normally such containers won't occur, but they can show up
                 * when two messages have References lines that disagree. For
                 * example, assuming A and B are messages, and 1, 2, and 3 are
                 * references for messages we haven't seen:
                 * 
                 * A has refs: 1 2 3
                 * B has refs: 1 3
                 * 
                 * There is ambiguity as to whether 3 is a child of 1 or 2. So,
                 * depending on the processing order, we might end up with
                 * either
                 *  -- 1
                 *     |-- 2
                 *         |-- 3
                ...
                // Don't do this if we would be promoting them to the root
                // level,
                ...
                // Remove this container from the list, replacing it with
                // `kids'.
                if (prev == null)
                    parent.child = kids;
                else
                ...
                // Since we've inserted items in the chain, `next' currently
                // points
                // to the item after them (tail.next); reset that so that we
                // process
    ...
    private void gatherSubjects() {
        ...
        for (ThreadContainer c = root_node.child; c != null; c = c.next) {
            ...
            // Only root set members may be dummies, and they always have at
            // least
            ...
            // one is more interesting as a root, so put it in the table
            // instead.
        ...
        for (prev = null, c = root_node.child, rest = c.next; 
             ...
             *  - If one container is a dummy and the other is not, make the
             * non-dummy one be a child of the dummy, and a sibling of the other
             * "real" messages with the same subject (the dummy's children.)
             *  - If that container is a non-dummy, and that message's subject
             ...
             *  - Otherwise, make a new dummy container and make both messages
             ...
             * suck.)
             * 
             * (It has occurred to me that taking the date or message number
             * into account would be one way of resolving some of the ambiguous
             * cases, but that's not altogether straightforward either.)
             */
            ...
            if (old.threadable == null && c.threadable == null) {
                ...
                c.child = null;
                    ...
                    (c.threadable != null && c.threadable.subjectIsReply() && // c
                                                                                // has
                                                                                // Re,
                                                                                // and
                 ...
                 * container. We do this by creating a new container object for
                 * old->msg and transforming the old container into a dummy (by
                 * merely emptying it), so that the hash table still points to
                 * the one that is at depth 0 instead of depth 1.
                 */
                ...
                // old is now a dummy; make it have exactly two kids, c and
                // newc.
                old.child = c;
                c.next = newc;
            }
...
/*
 * The ThreadContainer object is used to encapsulate an IThreadable object (it
 * holds some intermediate state used while threading.) This is a private class
 * that doesn't escape from this module.
 */
class ThreadContainer {
    ...
    void reverseChildren() {
        if (child != null) {
            ...
            for (prev = null, kid = child, rest = kid.next; kid != null; prev = kid, kid = rest, rest = (rest == null ? null
                    : rest.next))
                kid.next = prev;
            child = prev;
