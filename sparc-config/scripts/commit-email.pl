...
foreach my $project (@project_settings_list)
  {
    ...
    $svn_user = read_user_info();
    push(@head, "Subject: [SQO-OS]S SVN commit ($author - $rev)\n");
    push(@head, "Reply-to: $reply_to\n") if $reply_to;
    push(@head, "From: $svn_user\n");
    ...
    if (defined $sendmail and @email_addresses)
      {
        ...
        my $command = "$sendmail -f'$svn_user' $userlist";
...
# bkarak #############################################################################
# read the file /var/svn/common/conf/svnauthdesc and fetch email for the current user
# its a little quick and dirty (need optimisation at some point in the future)
...
sub read_user_info {
	...
	open(PFILE,"/var/svn/common/conf/svnauthdesc");
	@data = <PFILE>;
	close(PFILE);
	# find the user or return false
	foreach ( @data ) {
		@line = split(/:/);
		if($line[0] eq $author) {
			return $line[1]." <".trim($line[3]).">\n";
		}
	}
	print "invalid user <root\@sense.dmst.aueb.gr>";
}

# trim the line from any new line characters
sub trim{
    $_[0] =~ s/\n//g;
    return $_[0];
}
