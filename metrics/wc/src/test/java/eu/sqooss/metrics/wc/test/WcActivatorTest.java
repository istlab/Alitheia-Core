...
import eu.sqooss.metrics.wc.WcActivator;
...
@PrepareForTest({ DBService.class, AlitheiaCore.class, WcImplementation.class, WcActivator.class })
public class WcActivatorTest {

	static WcActivator testObject;
	...
	public void testStartStopMethod() throws Exception {
		...
		WcImplementation metrics  = mock(WcImplementation.class);
		PowerMockito.whenNew(WcImplementation.class).withAnyArguments().thenReturn(metrics);
		BundleContext mockedBC = mock(BundleContext.class);
		when(mockedBC.registerService(eq(WcImplementation.class.getName()), any(WcImplementation.class), any(Dictionary.class))).thenReturn(mock(ServiceRegistration.class));
		//Run the start test
